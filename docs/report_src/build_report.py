...
s.append(P(f"Ivan Grahek, {datetime.date.today():%d %B %Y}. Code and results: branches <font face='DVM'>issue-1-ou-param-recovery</font> and <font face='DVM'>kernel-ou-fit</font> of <font face='DVM'>rnn_hssm</font>. Software: HSSM 0.3 and 0.2.4, ssm-simulators 0.8.3, gain-controller-rnn at 1ca084b; Oscar jobs 6604819, 6604891, 6606075, 6606166, 6606270, 6606439.", SM))
s.append(Spacer(1, 6))
...
s.append(P("Throughout, g follows the convention of the simulator the LAN was trained on, dx = (v - g x) dt + dW: positive g is leaky, negative g is unstable. The original fits therefore sat at the unstable edge of the allowed range, not the leaky one."))
