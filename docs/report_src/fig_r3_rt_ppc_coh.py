...
from matplotlib.patches import Patch; from matplotlib.lines import Line2D; from matplotlib.ticker import FuncFormatter, MultipleLocator
...
for sf, b in zip(sfs, BOUNDS):
    ...
    for j, gn in enumerate(GAINS):
        ...
        for i, c in enumerate(COHS):
            ...
            ax.hist(rt[~cm], bins=bins, weights=-np.ones((~cm).sum()) / len(rt), histtype="step", color="k", lw=0.9)
            ax.axhline(0, color="k", lw=0.4); ax.set_xlim(0, 750)
