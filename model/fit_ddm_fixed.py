...
def main():
    ...
    try:
        ...
        if hasattr(result, 'figure'):
            result.figure.savefig(str(OUT / f"ddm_fixed_{tag}_ppc.png"), dpi=150, bbox_inches='tight')
        elif hasattr(result, 'savefig'):
            result.savefig(str(OUT / f"ddm_fixed_{tag}_ppc.png"), dpi=150, bbox_inches='tight')
        plt.close('all')
    ...
    try:
        ...
        ax.set_ylim(0.0, 1.2)
        ax.figure.savefig(str(OUT / f"ddm_fixed_{tag}_qpp.png"), dpi=150, bbox_inches='tight')
        plt.close('all')
        print("QPP saved.")
    except Exception as e:
        print(f"Quantile probability plot failed: {e}")

    out_path = OUT / f"ddm_fixed_{tag}"
